...
# Brand assets rather than dashboard screenshots: they live in the same
# directory because that is where the README's images are, and they are not
# regenerated by `make screenshots`.
BRAND = {"logo.png", "logo-256.png"}
...
def test_no_screenshot_is_left_over_from_a_dashboard_that_is_gone():
    ...
    expected = {f"{slug(dash['name'])}.png" for dash in DASHBOARDS} | BRAND
...
def test_the_brand_assets_are_there_and_are_not_enormous():
    """The logo is a 1254px sticker at source, which is not what belongs in a
    README. Quantised and resized it is under a tenth of that."""
    for name in sorted(BRAND):
        path = IMAGES / name
        assert path.exists(), f"{name} is missing"
        size = path.stat().st_size
        assert size < 200_000, f"{name} is {size // 1024} kB — resize or quantise it"
        assert size > 2_000, f"{name} is {size} bytes"
